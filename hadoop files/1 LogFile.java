	...
	public static class MapperLogFile extends Mapper<LongWritable,Text,Text,IntWritable>{
		public void map(LongWritable key,Text value,Context con) throws IOException,InterruptedException{
			...
			for(String currLine: lines){
				...
				String intimeString=splitLine[5];
				String outtimeString=splitLine[7];
				String intime=intimeString.split(" ")[1];
				String outtime=outtimeString.split(" ")[1];
...
for(String currLine: lines)
{
	...
	long difference = outdate.getTime() - intime.getTime();
	int minutes = (int) ((difference/(60*1000))%60);
	Text outputKey = new Text(ipaddress.toUpperCase().trim());
	IntWritable outvalue = new IntWritable(minutes);
	con.write(outputKey,outvalue);
}
...
int max_value = 0;
HashMap<String,Integer> timeValues = new HashMap<>();
public void reduce(Text ip, Iterable<IntWritable> times, Context con) throws IOException, InterruptedException {
	int totalTime = 0;
	for(IntWritable num:times)
	{
		totalTime +=num.get();
